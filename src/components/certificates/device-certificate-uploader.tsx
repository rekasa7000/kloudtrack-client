...
import {
  CertificateFile,
  DeviceCertificateUploaderProps,
...
const DeviceCertificateUploader: React.FC<DeviceCertificateUploaderProps> = ({
  ...
  const handleFileUpload = async (
    ...
    const expectedExtensions =
      type === "privateKey" ? /\.(pem|key)$/i : /\.(pem|crt|cer)$/i;

    if (!file.name.match(expectedExtensions)) {
      const expectedFormats =
        type === "privateKey" ? ".pem, .key" : ".pem, .crt, .cer";
      setErrors((prev) => ({
        ...prev,
    ...
    try {
      ...
      const validator =
        type === "privateKey" ? validatePrivateKey : validateCertificate;
  ...
  const renderUploadArea = (
    ...
    <div className="space-y-4">
      ...
      {!file ? (
        <div
          className={`border-2 border-dashed flex flex-col justify-center rounded-lg p-4 text-center transition-colors aspect-[9.7/4] ${
            dragStates[type]
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-1">
            Drop your {type === "privateKey" ? "private key" : "certificate"}{" "}
            here, or{" "}
            <button
        ...
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center gap-2 text-green-800">
            ...
            <span className="font-medium">
              Both certificates uploaded successfully!
            </span>
          </div>
          <p className="text-sm text-green-700 mt-1">
            Your device is ready to connect to AWS IoT Core.
          </p>
        </div>
      )}
    </div>

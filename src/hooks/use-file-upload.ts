...
export const useFileUpload = (options: FileUploadOptions = {}): [FileUploadState, FileUploadActions] => {
  ...
  const createPreview = useCallback((file: File | FileMetadata): string | undefined => {
    if (file instanceof File) {
      // Only create object URL for File objects (Blob-like objects)
      return URL.createObjectURL(file);
    }
    // For FileMetadata, return the existing URL
    return file.url;
  }, []);
  ...
  const clearFiles = useCallback(() => {
    setState((prev) => {
      // Clean up object URLs only for File objects
      prev.files.forEach((fileWithPreview) => {
        if (
